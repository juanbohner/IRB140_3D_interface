...
# ------------------------- Límites articulares -------------------------
LIMIT_JOINTS = True  # por defecto respeta los límites
JOINT_LIMITS = [
...
class ManipulabilityAppGL(QtWidgets.QWidget):
    def __init__(self):
        ...
        # default scale: assume stl in mm -> convert to meters
        self.stl_scale = 0.001
        ...
        # Checkbox para mostrar STL
        self.show_stl_chk = QtWidgets.QCheckBox("Mostrar STL")
        self.show_stl_chk.setChecked(True)  # arranca apagado
        self.show_stl_chk.stateChanged.connect(self.toggle_stl_visibility)
        ctrl.addWidget(self.show_stl_chk)
        ...
        # Axis limit checkbox (implementado como cámara fija)
        ...
        # --- Panel de offsets STL (editable en runtime) ---
        self.offset_widgets = []  # para cada link: dict de spinboxes
        self.stl_files = [
            "visual/IRB140_-_M2004C_BASE.STL",
            "visual/IRB140_-_M2004C_LINK1.STL",
            "visual/IRB140_-_M2004C_LINK2.STL",
            "visual/IRB140_-_M2004C_LINK3.STL",
            "visual/IRB140_-_M2004C_LINK4.STL",
            "visual/IRB140_-_M2004C_LINK5.STL",
            "visual/IRB140_-_M2004C_LINK6.STL"
        ]

        gb_offsets = QtWidgets.QGroupBox("Offsets STL (m, deg)")
        voff = QtWidgets.QVBoxLayout()
        gb_offsets.setLayout(voff)
        ctrl.addWidget(gb_offsets)

        # Buttons to save/load/reset offsets
        h_off_btns = QtWidgets.QHBoxLayout()
        self.save_off_btn = QtWidgets.QPushButton("Guardar offsets")
        self.save_off_btn.clicked.connect(self.save_offsets_dialog)
        self.load_off_btn = QtWidgets.QPushButton("Cargar offsets")
        self.load_off_btn.clicked.connect(self.load_offsets_dialog)
        self.reset_off_btn = QtWidgets.QPushButton("Reset offsets")
        self.reset_off_btn.clicked.connect(self.reset_offsets)
        h_off_btns.addWidget(self.save_off_btn)
        h_off_btns.addWidget(self.load_off_btn)
        h_off_btns.addWidget(self.reset_off_btn)
        voff.addLayout(h_off_btns)

        # Scale control (in case STLs are in different units)
        hscale = QtWidgets.QHBoxLayout()
        hscale.addWidget(QtWidgets.QLabel("Escala STL (1 = metros, 0.001 = mm->m):"))
        self.scale_spin = QtWidgets.QDoubleSpinBox()
        self.scale_spin.setRange(1e-6, 10.0)
        self.scale_spin.setDecimals(6)
        self.scale_spin.setSingleStep(0.001)
        self.scale_spin.setValue(self.stl_scale)
        self.scale_spin.valueChanged.connect(self.change_scale)
        hscale.addWidget(self.scale_spin)
        voff.addLayout(hscale)

        # Toggle show axes for each STL
        self.show_stl_axes_chk = QtWidgets.QCheckBox("Mostrar ejes origen STL")
        self.show_stl_axes_chk.setChecked(False)
        self.show_stl_axes_chk.stateChanged.connect(self.update_scene)
        voff.addWidget(self.show_stl_axes_chk)

        # Scroll area for individual offsets
        ...
        voff.addWidget(scroll)

        # Default offsets (identity) for each STL (SE3)
        self.OFFSETS = [SE3() for _ in self.stl_files]

        for i, fname in enumerate(self.stl_files):
            label, widget, wdict = self._make_offset_row(i, fname)
            form.addRow(label, widget)
            self.offset_widgets.append(wdict)
        ...
        # --- Cargar STL de los links (raw + visual) ---
        self.link_meshes = []
        self.stl_raw = []  # guardar mesh.Mesh originales para reconstruir con distinta escala
        for f in self.stl_files:
            if not os.path.exists(f):
                print(f"Archivo STL no encontrado: {f}")
                self.stl_raw.append(None)
                self.link_meshes.append(None)
                continue
            try:
                m = mesh.Mesh.from_file(f)
                self.stl_raw.append(m)
                item = self._create_meshitem_from_stl(m, scale=self.stl_scale)
        ...
        # Axis helpers for STL origins (list of triplets of GLLinePlotItem)
        self.stl_axes = [None] * len(self.stl_files)
    ...
    # ---------------- UI helpers for offsets ----------------
    def _make_offset_row(self, idx, fname):
        label = QtWidgets.QLabel(f"[{idx}] {os.path.basename(fname)}")
        widget = QtWidgets.QWidget()
        h = QtWidgets.QHBoxLayout()
        widget.setLayout(h)

        tx = QtWidgets.QDoubleSpinBox(); tx.setRange(-2.0, 2.0); tx.setSingleStep(0.001); tx.setDecimals(4); tx.setValue(0.0)
        ty = QtWidgets.QDoubleSpinBox(); ty.setRange(-2.0, 2.0); ty.setSingleStep(0.001); ty.setDecimals(4); ty.setValue(0.0)
        tz = QtWidgets.QDoubleSpinBox(); tz.setRange(-2.0, 2.0); tz.setSingleStep(0.001); tz.setDecimals(4); tz.setValue(0.0)
        rx = QtWidgets.QDoubleSpinBox(); rx.setRange(-180.0, 180.0); rx.setSingleStep(0.5); rx.setValue(0.0)
        ry = QtWidgets.QDoubleSpinBox(); ry.setRange(-180.0, 180.0); ry.setSingleStep(0.5); ry.setValue(0.0)
        rz = QtWidgets.QDoubleSpinBox(); rz.setRange(-180.0, 180.0); rz.setSingleStep(0.5); rz.setValue(0.0)

        for sb in (tx,ty,tz,rx,ry,rz):
            sb.setFixedWidth(70)
            sb.valueChanged.connect(lambda _val, i=idx: self._offset_widget_changed(i))

        h.addWidget(QtWidgets.QLabel('t(m):'))
        h.addWidget(tx); h.addWidget(ty); h.addWidget(tz)
        h.addWidget(QtWidgets.QLabel('r(deg):'))
        h.addWidget(rx); h.addWidget(ry); h.addWidget(rz)

        return label, widget, {'tx':tx,'ty':ty,'tz':tz,'rx':rx,'ry':ry,'rz':rz}

    def _offset_widget_changed(self, idx):
        # Leer widgets y actualizar self.OFFSETS
        w = self.offset_widgets[idx]
        tx = w['tx'].value(); ty = w['ty'].value(); tz = w['tz'].value()
        rx = np.deg2rad(w['rx'].value()); ry = np.deg2rad(w['ry'].value()); rz = np.deg2rad(w['rz'].value())
        # Orden de rotaciones: Rx, then Ry, then Rz (aplicadas después de la traslación)
        R = SE3.Rx(rx) * SE3.Ry(ry) * SE3.Rz(rz)
        T = SE3(tx, ty, tz) * R
        self.OFFSETS[idx] = T
        # Actualizar visual
        self.update_scene()

    def save_offsets_dialog(self):
        fn, _ = QtWidgets.QFileDialog.getSaveFileName(self, 'Guardar offsets', '', 'JSON Files (*.json)')
        if not fn:
            return
        data = []
        for i, T in enumerate(self.OFFSETS):
            if i < len(self.offset_widgets):
                w = self.offset_widgets[i]
                data.append([w['tx'].value(), w['ty'].value(), w['tz'].value(), w['rx'].value(), w['ry'].value(), w['rz'].value()])
            else:
                data.append(None)
        try:
            with open(fn, 'w') as f:
                json.dump(data, f, indent=2)
            QtWidgets.QMessageBox.information(self, 'Guardar', f'Offsets guardados en {fn}')
        except Exception as e:
            QtWidgets.QMessageBox.warning(self, 'Error', f'No se pudo guardar offsets: {e}')

    def toggle_stl_visibility(self, state):
    ...
    def load_offsets_dialog(self):
        fn, _ = QtWidgets.QFileDialog.getOpenFileName(self, 'Cargar offsets', '', 'JSON Files (*.json)')
        if not fn:
            return
        try:
            with open(fn, 'r') as f:
            ...
            for i, vals in enumerate(data):
                ...
                if i < len(self.offset_widgets):
                    w = self.offset_widgets[i]
                    w['tx'].setValue(float(vals[0])); w['ty'].setValue(float(vals[1])); w['tz'].setValue(float(vals[2]))
                    w['rx'].setValue(float(vals[3])); w['ry'].setValue(float(vals[4])); w['rz'].setValue(float(vals[5]))
                    # update OFFSETS
                    self._offset_widget_changed(i)
            QtWidgets.QMessageBox.information(self, 'Cargar', f'Offsets cargados desde {fn}')
        except Exception as e:
            QtWidgets.QMessageBox.warning(self, 'Error', f'No se pudo cargar offsets: {e}')

    def reset_offsets(self):
        for i, w in enumerate(self.offset_widgets):
            w['tx'].setValue(0.0); w['ty'].setValue(0.0); w['tz'].setValue(0.0)
            w['rx'].setValue(0.0); w['ry'].setValue(0.0); w['rz'].setValue(0.0)
        self.OFFSETS = [SE3() for _ in self.stl_files]
        self.update_scene()

    def change_scale(self, val):
        self.stl_scale = float(val)
        # reconstruir todos los meshdata
        for i, m in enumerate(self.stl_raw):
            if m is None:
                continue
            try:
                md = self._meshdata_from_stl_mesh(m, scale=self.stl_scale)
                if self.link_meshes[i] is None:
                    item = gl.GLMeshItem(meshdata=md, smooth=True, color=(0.7,0.7,0.7,1.0), shader="shaded", glOptions="opaque")
                    self.view.addItem(item)
                    self.link_meshes[i] = item
                else:
                    self.link_meshes[i].setMeshData(meshdata=md)
            except Exception as e:
                print(f"Error al reescalar STL {self.stl_files[i]}: {e}")
        self.update_scene()

    def _meshdata_from_stl_mesh(self, m, scale=0.001):
        # m.v0, m.v1, m.v2 tienen shape (n,3)
        n = len(m.v0)
        verts = np.concatenate((m.v0, m.v1, m.v2), axis=0).astype(np.float32) * float(scale)
        faces = np.arange(3 * n, dtype=np.int32).reshape(n, 3)
    ...
    def _create_meshitem_from_stl(self, m, scale=0.001):
        md = self._meshdata_from_stl_mesh(m, scale=scale)
        item = gl.GLMeshItem(
            meshdata=md,
            smooth=True,
        ...
        try:
            for i, item in enumerate(self.link_meshes):
                if item is None:
                    continue
                if i < len(Ts):
                    # Aplicar offset sobre el frame del eslabón
                    T_link = Ts[i]
                    if isinstance(T_link, np.ndarray):
                        T_arr = T_link
                    else:
                        T_arr = T_link.A
                    Off = self.OFFSETS[i].A if self.OFFSETS[i] is not None else np.eye(4)
                    T_tot = T_arr @ Off
                    ...
                    # Traslación (en metros)
                    item.translate(t[0], t[1], t[2])

                    # Rotación → convertir matriz a eje-ángulo
                    angle_rad = np.arccos(max(min((np.trace(R) - 1) / 2, 1.0), -1.0))
                    if abs(angle_rad) > 1e-6:
                        ...
                        item.rotate(math.degrees(angle_rad), axis[0], axis[1], axis[2])


                    # actualizar ejes locales del STL si están visibles
                    if self.show_stl_axes_chk.isChecked():
                        p = T_tot[:3, 3].astype(np.float32)
                        R = T_tot[:3, :3]
                        axis_len = 0.06
...
# -------------------------------------------------------------------------
# Notas rápidas:
# - Escala por defecto: 0.001 (mm -> m). Cambiala si tus STLs ya vienen en metros.
# - En el panel izquierdo tenés controles tx,ty,tz (m) y rx,ry,rz (grados) por cada STL.
# - El botón \"Mostrar ejes origen STL\" dibuja flechas en el origen que aplicaste con el offset.
# - Guardá offsets con \"Guardar offsets\" y recargalos con \"Cargar offsets\".
# - Si un STL se ve roto en el viewer, revisalo en MeshLab/Blender: este código respeta las caras (m.v0,m.v1,m.v2).
# -------------------------------------------------------------------------
