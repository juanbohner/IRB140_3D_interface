"""
IRB140 - Visualizador de elipsoide de manipulabilidad (traslacional)
Modificado: Añade selección de punto de origen y destino (absolutos), movimiento articular hasta el origen y trayectoria rectilínea TCP entre origen y destino.

Cambios principales:
 - Nuevo botón para introducir ORIGEN y DESTINO en coordenadas TCP (x,y,z) en metros.
 - Si origen está vacío se usa la posición TCP actual.
 - Para alcanzar el ORIGEN se resuelve IK iterativo para obtener q_origin y luego se genera una interpolación articular (joint-space) desde la configuración actual hasta q_origin (esto produce el movimiento articular pedido).
 - A partir de q_origin se genera la trayectoria rectilínea en TCP hacia DESTINO usando la función generate_linear_trajectory existente.
 - En la vista 3D se dibuja la trayectoria planificada (línea recta entre origen y destino) y puntos de muestreo que el robot recorrerá.
 - Los pasos articulares iniciales (move-to-origin) y la trayectoria cartesiana se concatenan en self.traj para su reproducción con Play.

Nota: mantuve intactas las funciones de FK y Jacobianas originales y usé las rutinas DLS adaptativas ya implementadas para los dos segmentos (joints->origin y origin->dest).
"""
...
# ------------------------- Intentar usar Robotics Toolbox -------------------------
USE_RTB = True
try:
...
# ------------------------- Fallback DH -------------------------
if not USE_RTB or not RTB_HAS_IRB140:
    DH_PARAMS = [
        (0.0, -np.pi/2, 0.290, 0.0),
        (0.270, 0.0, 0.0, 0.0),
        (0.070, -np.pi/2, 0.0, 0.0),
        (0.0, np.pi/2, 0.302, 0.0),
        (0.0, -np.pi/2, 0.0, 0.0),
        (0.0, 0.0, 0.072, 0.0),
    ]
    DOF = len(DH_PARAMS)
    ...
    def forward_kinematics(joint_values):
        ...
        for i, (a, alpha, d, theta_off) in enumerate(DH_PARAMS):
            theta = joint_values[i] + theta_off
    ...
    def numeric_jacobian(joint_values, eps=1e-8):
        n = len(joint_values)
        T0, _ = forward_kinematics(joint_values)
        p0 = T0[:3, 3]
        Jv = np.zeros((3, n))
        for i in range(n):
            dq = np.zeros(n)
            dq[i] = eps
            T1, _ = forward_kinematics(joint_values + dq)
            p1 = T1[:3, 3]
            Jv[:, i] = (p1 - p0) / eps
        return Jv
...
# ------------------------- Manipulability y DLS adaptativo -------------------------
...
def plot_robot_and_ellipsoid(ax, joint_values, highlight_singular=False, planned_path_pts=None, planned_markers=None):
    ...
    # plot planned straight-line path if provided
    ...
    # plot special markers (origin/destination)
...
# ------------------------- Helper: generar trayectoria rectilínea en posición TCP usando IK incremental
...
# ------------------------- New helper: IK solve to reach a desired TCP point (returns q)
...
class ManipulabilityApp(QtWidgets.QWidget):
    def __init__(self):
        ...
        # NUEVO: atributos para mantener la línea de trayectoria visible
        ...
        # Trajectory controls - nuevo botón con origen y destino absolutos
    ...
    def generate_trajectory_with_origin_dest(self):
        ...
        dlg.setLabelText('Ingrese ORIGEN x,y,z (o vacío para usar TCP actual) ; DESTINO x,y,z separados por ";"\nEj: 0.2,0,0 ; 0.5,0.1,0.2')
        ok = dlg.exec_()
        if not ok:
            return
        text = dlg.textValue()
        try:
            ...
            if len(parts) == 1:
                # si solo hay un punto, lo tomamos como destino y origen=current
                origin_text = ''
                dest_text = parts[0]
            else:
                origin_text = parts[0]
            ...
            QtWidgets.QMessageBox.warning(self, 'Error', 'Formato inválido. Use: x,y,z ; x,y,z')
            return

        # Paso 1: si el origen no coincide con el TCP actual, calcular q_origin mediante IK
        ...
        if np.linalg.norm(p0 - p_tcp) > 1e-6:
            ...
            if not ok_conv:
                QtWidgets.QMessageBox.warning(self, 'IK', f'No se pudo converger al origen en {its} iter. Se usará la mejor aproximación.')
            # generar interpolación articular desde q_start hasta q_origin
            steps_js = 120
            for t in np.linspace(0, 1, steps_js):
                q_interp = (1.0 - t) * q_start + t * q_origin
                # calcular métricas para consistencia
            ...
            # set q for next stage to q_origin
            q_for_cartesian = q_origin.copy()
        else:
            # origen = TCP actual
            q_for_cartesian = q_start.copy()

        # Paso 2: generar trayectoria rectilínea cartesiana desde p0 a pd
        steps_cart = 200
        ...
        # almacenar trayectoria y dibujar la trayectoria planificada en el plot
        self.traj = full_traj
        self.traj_idx = 0

        # calcular puntos para plot (muestras entre p0 y pd)
        ...
        QtWidgets.QMessageBox.information(self, 'Trayectoria', f'Trayectoria generada: {len(full_traj)} pasos. ({len(joint_steps)} articular + {len(traj_cart)} cartesiano). Pulse Play para reproducir.')

        # calcular puntos para plot (muestras entre p0 y pd)
        self.planned_pts = [ (1.0 - t) * p0 + t * pd for t in np.linspace(0,1,60) ]
        self.planned_markers = [('Origen', p0), ('Destino', pd)]


        cond, svals = plot_robot_and_ellipsoid(
        self.ax, self.joints,
        planned_path_pts=self.planned_pts,
        planned_markers=self.planned_markers
        )
        self.canvas.draw()
        ...
        self, 'Trayectoria',
        f'Trayectoria generada: {len(full_traj)} pasos. ({len(joint_steps)} articular + {len(traj_cart)} cartesiano). Pulse Play para reproducir.'
        )
    ...
    def _play_step(self):
        ...
        cond_pos, _ = plot_robot_and_ellipsoid(
            self.ax,
            self.joints,
            planned_path_pts=self.planned_pts, # mantener visible
            planned_markers=self.planned_markers # mantener visible
        )
