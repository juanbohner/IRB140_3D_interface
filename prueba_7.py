...
def manipulability_ellipsoid_from_J(Jv):
    try:
        ...
        return np.array([0,0,0]), np.eye(3), np.eye(3), np.array([]), np.inf

    # Los ejes están en U, con longitudes = svals
    axes_len = svals
    axes_dir = U

    # Matriz de manipulabilidad M = J J^T
    M = Jv @ Jv.T
    ...
    return axes_len, axes_dir, M, svals, cond
...
def plot_robot_and_ellipsoid(ax, joint_values, highlight_singular=False, planned_path_pts=None, planned_markers=None):
    # limpiar completamente el eje
    ax.cla()
    ax.set_box_aspect([1,1,1])
    ax.grid(True)

    # Revisar si hay que usar límites fijos o auto-escalado
    parent_app = getattr(ax.figure.canvas, "parent", lambda: None)()
    use_fixed_limits = True
    if parent_app and hasattr(parent_app, "axis_limit_chk"):
        use_fixed_limits = parent_app.axis_limit_chk.isChecked()
    ...
    if use_fixed_limits:
        ...
        ax.set_ylim(-lim, lim)
        ax.set_zlim(0, 1.2)
    # si no hay límites fijos, se dejará que Matplotlib haga autoscale al final
...
class ManipulabilityApp(QtWidgets.QWidget):
    def __init__(self):
        ...
        # Checkbox para activar/desactivar límites fijos
        self.axis_limit_chk = QtWidgets.QCheckBox("Usar límites fijos en ejes")
        self.axis_limit_chk.setChecked(True)  # por defecto activado
    ...
    def set_view(self, mode):
        """Cambia la vista de la cámara en el gráfico 3D."""
        if mode == 'top':       # superior
            self.ax.view_init(elev=90, azim=-90)
        elif mode == 'front':   # frontal
            self.ax.view_init(elev=0, azim=0)
        elif mode == 'side':    # lateral
            self.ax.view_init(elev=0, azim=90)
        elif mode == 'iso':     # isométrica
