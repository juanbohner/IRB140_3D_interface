...
class ManipulabilityAppGL(QtWidgets.QWidget):
    ...
    #     # Robot line
    #     self.robot_line = gl.GLLinePlotItem(width=5, antialias=True, color=(0,0,1,1), mode='line_strip')
    ...
    #     #Terna de la TOOL (tena 6)
    #     self.tcp_axis_x = gl.GLLinePlotItem(color=(0,0.5,1,1), width=3)  # azul claro
    #     self.tcp_axis_y = gl.GLLinePlotItem(color=(0,0.5,1,1), width=3)
    #     self.tcp_axis_z = gl.GLLinePlotItem(color=(0,0.5,1,1), width=3)
    ...
    #     self.ellipsoid_vecs = [
    #         gl.GLLinePlotItem(color=(0,1,1,1), width=2),  # cian
    #         gl.GLLinePlotItem(color=(0,1,1,1), width=2),
    #         gl.GLLinePlotItem(color=(0,1,1,1), width=2),
    #     ]
    ...
    #     self.planned_line = gl.GLLinePlotItem(pos=np.zeros((2,3)), width=2, antialias=True, color=(1,1,0,1))  # amarillo
    #     self.view.addItem(self.planned_line)
    #     self.planned_scatter = gl.GLScatterPlotItem(pos=np.zeros((1,3)), size=4, pxMode=True, color=(0,1,0,1))  # puntos verdes
    #     self.view.addItem(self.planned_scatter)

    #     ##      Puntos de los extremos de la recta
    #     self.origin_scatter = gl.GLScatterPlotItem(pos=np.zeros((1,3)), size=10, pxMode=True, color=(1,0,0,1))  # esfera roja origen
    #     self.view.addItem(self.origin_scatter)
    #     self.dest_scatter = gl.GLScatterPlotItem(pos=np.zeros((1,3)), size=10, pxMode=True, color=(1,0,0,1))    # esfera roja destino
    ...
    def make_slider_callback(self, idx, label):
        def cb(val):
            ...
            if self.planned_pts is not None:
                self.planned_pts = None
                self.planned_markers = None
                self.planned_line.setData(pos=np.zeros((2,3)))
                self.planned_scatter.setData(pos=np.zeros((1,3)))
    ...
    #     self.robot_line.setData(pos=origins.astype(np.float32))
    #     self.joint_scatter.setData(pos=origins.astype(np.float32), size=8, pxMode=True)

    #     # Terna 6 (TOOL)
    #     self.tcp_axis_x.setData(pos=np.array([p, p+0.1*R[:,0]]))
    #     self.tcp_axis_y.setData(pos=np.array([p, p+0.1*R[:,1]]))
    #     self.tcp_axis_z.setData(pos=np.array([p, p+0.1*R[:,2]]))


    #     # Planned trajectory
    #     if self.planned_pts is not None and len(self.planned_pts) > 0:
    #         pts = np.array(self.planned_pts)
    #         self.planned_line.setData(pos=pts, mode='line_strip')
    #         self.planned_scatter.setData(pos=pts[1:-1])   # puntos verdes intermedios
    #         self.origin_scatter.setData(pos=np.array([pts[0]]))   # esfera roja origen
    #         self.dest_scatter.setData(pos=np.array([pts[-1]]))    # esfera roja destino
    #     else:
    #         self.planned_line.setData(pos=np.zeros((2,3)))
    #         self.planned_scatter.setData(pos=np.zeros((1,3)))
    #         self.origin_scatter.setData(pos=np.zeros((1,3)))
    #         self.dest_scatter.setData(pos=np.zeros((1,3)))
    ...
    #     p = Ttcp[:3,3]   # posición TCP
    #     R = Ttcp[:3,:3]  # rotación TCP


    #     length = 0.1  # largo de los ejes
    #     tcp_axes = np.array([
    #         [p, p + length*R[:,0]],  # X
    #         [p, p + length*R[:,1]],  # Y
    #         [p, p + length*R[:,2]],  # Z
    ...
    #     # generar esfera parametric y transformarla con U*Sigma
    #     nu = 36
    #     nv = 18
    ...
    #             verts.append(p + point)
    #     verts = np.array(verts)

    #     # faces (triangulación simple)
    #     faces = []
    #     for i in range(nu-1):
    #         for j in range(nv-1):
    #             a = i*nv + j
    ...
    #         color = (0.35, 0.75, 0.0, 0.55)  # naranja claro con alpha=0.2
    ...
    #         )
    #         self.ellipsoid_mesh_item.setColor(color)
    ...
    #     # # Adjust camera if "fixed" selected
    #     # if self.axis_limit_chk.isChecked():
    #     #     # cámara fija: distancia y centro estáticos
    #     #     self.view.opts['distance'] = 1.2
    #     #     self.view.opts['center'] = pg.Vector(p[0], p[1], p[2])
    #     #     self.view.setCameraPosition(distance=1.2)

    #     # else:
    #     #     # autoscale-like: center camera at TCP and increase distance depending on reach
    #     #     reach = max(np.linalg.norm(origins - p, axis=1).max(), 0.3)
    #     #     dist = 0.8 + reach
    ...
    #     # compute lam used by current J (for display)
    #     try:
    ...
    def update_scene(self, initial=False):
        ...
        if self.planned_pts is not None and len(self.planned_pts) > 0:
            ...
            # entire line
            self.planned_line.setData(pos=pts, mode='line_strip')
            # intermediate green points (exclude first and last if length>2)
            if pts.shape[0] > 2:
                self.planned_scatter.setData(pos=pts[1:-1], size=4, pxMode=True)
            else:
                self.planned_scatter.setData(pos=np.zeros((1, 3), dtype=np.float32), size=4, pxMode=True)
            # endpoints as red spheres
            ...
            # update 2D labels on panel
            self.origin_label.setText(f"Origen: {np.round(pts[0], 3)}")
            self.dest_label.setText(f"Destino: {np.round(pts[-1], 3)}")
        else:
            # keep the last displayed line if you prefer persistency.
            # if you prefer to hide when None, uncomment the two lines below:
        ...
        # build and draw ellipsoid mesh (recreate meshitem)
        try:
            p = p.astype(np.float32)
            nu = 28  # reduce resolution para perf
            nv = 14
            u = np.linspace(0, 2 * np.pi, nu)
            v = np.linspace(0, np.pi, nv)
            X = np.outer(np.cos(u), np.sin(v))
            ...
            for i in range(nu):
                for j in range(nv):
                    ...
                    # si axes_len es array de 3 (σ), axes_dir es 3x3
                    point = (axes_len[0] * vec[0] * axes_dir[:, 0] +
                            axes_len[1] * vec[1] * axes_dir[:, 1] +
                            axes_len[2] * vec[2] * axes_dir[:, 2])
            ...
            # elipsoide color verde claro, algo translúcido
            self.ellipsoid_mesh_item = gl.GLMeshItem(meshdata=meshdata, smooth=True, drawFaces=True, drawEdges=False,
                                                    shader='shaded', glOptions='translucent')
            # setColor acepta RGBA floats
            self.ellipsoid_mesh_item.setColor((0.45, 0.9, 0.3, 0.20))
            self.ellipsoid_mesh_item.setGLOptions('translucent')
            self.view.addItem(self.ellipsoid_mesh_item)
        except Exception:
            # no dibujes el mesh si falla
            pass
    ...
    def _play_step(self):
        ...
        self.update_scene()
        # actualizar info_label con valores del paso
